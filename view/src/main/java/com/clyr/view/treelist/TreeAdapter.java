...
public class TreeAdapter extends BaseAdapter {
    ...
    private final List<Node> all = new ArrayList<>();//展示
    private final List<Node> cache = new ArrayList<>();//缓存
    private final TreeAdapter tree = this;
    boolean hasCheckBox;
    ...
    public void setExpandLevel(int level) {
        ...
        for (int i = 0; i < cache.size(); i++) {
            Node n = cache.get(i);
            if (n.getLevel() <= level) {
                n.setExplaned(n.getLevel() < level);
                all.add(n);
            }
        }
...
//			vi.cb.setChecked(n.isChecked());
            ...
            if (vi != null) {
                if (n.isLeaf()) {
                    ...
                    vi.flagIcon.setOnClickListener(v -> ExpandOrCollapse(location));
                    if (n.isExplaned()) {
                        if (expandIcon != -1) {
